...
# 智能代理：自动检测运行环境，Docker容器内无Clash，不设代理
if not os.path.exists("/.dockerenv"):
    # 本地宿主机开发环境，启用Clash代理隧道
...
from gateways.perception import NapCatPerception, NativeUIAPerception # 挂载感知器官，结构化感知核心
    ...
    def try_initialize_subsystems(self) -> bool:
        ...
        try:
            ...
            with self.neo4j.session() as session:
                ...
                self.creator_id = os.getenv("CREATOR_QQ")
                print(Fore.GREEN + f"  ✅ 身份确认：欢迎回来，造物主 {self.creator_name} (内部索引: {self.creator_id})。")
                print(Fore.MAGENTA + Style.BRIGHT + "\n==================================================")
                print(Fore.MAGENTA + Style.BRIGHT + "  [System Status] 青青子衿，悠悠我心。纵我不往，子宁不嗣音？神经链路已全线重连。")
                print(Fore.MAGENTA + Style.BRIGHT + "==================================================\n")
            ...
            self.qq_perception = NapCatPerception(ws_url=os.environ.get("NAPCAT_WS", "ws://napcat:3001"), creator_qq=self.creator_id)
        ...
        while not is_ready:
            ...
            if not is_ready:
                if not navigated:
                    ...
                    print(Fore.YELLOW + "请前往浏览器完成参数配置。正等待同步...")
                    
                    # 尝试引导打开 Napcat (扫码) 和 Frontend (操作界面)
                    import webbrowser
                    try:
                        # 1. 先打开前端
